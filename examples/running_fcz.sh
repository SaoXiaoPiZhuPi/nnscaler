...
source /data/haiqwa/zevin_nfs/zhl-tuner/miniconda3/etc/profile.d/conda.sh
conda activate /opt/conda/envs/andy/nnscaler/
...
pushd /data/haiqwa/zevin_nfs/andy/Auto-Parallelization/nnscaler_group1/nnscaler-new/examples
DTIME=`date +%m-%d`
MTIME=`date +%H-%M`
...
data=$(cat running_fcz.conf)
declare $data
for PLAN_NGPUS in 4;do
    export LOG_NAME=${PROFILE_OUTPUT}/${MTIME}.dp_size_$((${RUNTIME_NGPUS} / ${PLAN_NGPUS}))_${NAME}.log
    if [ $# -le 1 ]; then
