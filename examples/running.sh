...
for PLAN_NGPUS in 4;do
    ...
    if [ $# -le 1 ]; then
        ...
        if [ "$PIPLINE" == "True" ]; then
            torchrun --nnodes=${NNODES} --nproc_per_node=${NPROC_PER_NODE} ${CODE} --run_mode run --plan_ngpus ${PLAN_NGPUS} --runtime_ngpus ${RUNTIME_NGPUS} --name ${NAME}_${PLAN_NGPUS} --model_id ${MODEL_ID} --dataset_path ${DATASET_PATH} --explore_pipeline 2>&1 | tee ${LOG_NAME}
        else
        ...
        if [ "$PIPLINE" == "True" ]; then
            torchrun --master_addr=$MASTER_ADDR --master_port=29502 --node_rank=$NODE_RANK --nnodes=${NNODES} --nproc_per_node=${NPROC_PER_NODE} ${CODE} --plan_ngpus ${PLAN_NGPUS} --runtime_ngpus ${RUNTIME_NGPUS} --name ${NAME}_${PLAN_NGPUS} --model_id ${MODEL_ID} --dataset_path ${DATASET_PATH} --explore_pipeline 2>&1 | tee ${LOG_NAME}
        else
            torchrun --master_addr=$MASTER_ADDR --master_port=29502 --node_rank=$NODE_RANK --nnodes=${NNODES} --nproc_per_node=${NPROC_PER_NODE} ${CODE} --plan_ngpus ${PLAN_NGPUS} --runtime_ngpus ${RUNTIME_NGPUS} --name ${NAME}_${PLAN_NGPUS} --model_id ${MODEL_ID} --dataset_path ${DATASET_PATH} 2>&1 | tee ${LOG_NAME}
        fi
