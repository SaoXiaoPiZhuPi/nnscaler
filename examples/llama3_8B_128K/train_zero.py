...
class WrapperModel(torch.nn.Module):
    ...
    def forward(self, samples):
        ...
        losses = chunk_linear_cross_entropy(hidden_states, self.model.lm_head.weight, samples['target'], IGNORE_IDX, 512)
...
def main(args):
    ...
    # 打印出内存高峰值
    print(f"Max memory allocated on CUDA: {torch.cuda.max_memory_allocated() / 1024**3:.2f} GiB")
    print(f"Max memory reserved on CUDA: {torch.cuda.max_memory_reserved() / 1024**3:.2f} GiB")
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default='time',
        choices=['time', 'mem'],
        help='dp priority of time or mem',
    )
