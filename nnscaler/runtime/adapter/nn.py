...
class ReduceBroadcast(torch.autograd.Function):
    ...
    def forward(ctx, input_: torch.Tensor, dst: int, ranks: List[int]):
        ...
        tensor_size = _get_tensor_size(input_)
        CudaTimer().start(field_name='reduce', predefined=True, comm_ranks=ranks, comm_size=tensor_size)
        torch.distributed.reduce(input_, dst, group=group)
        CudaTimer().stop(field_name='reduce', predefined=True)
        return input_
    ...
    def backward(ctx, grad_output):
        ...
        tensor_size = _get_tensor_size(grad_output)
