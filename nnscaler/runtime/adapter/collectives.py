...
def _get_tensor_size(tensor: Optional[torch.Tensor]) -> int:
    """获取 tensor 的字节数"""
...
def move(tensor: Optional[torch.Tensor], shape: Tuple[int], dtype: torch.dtype, src: int, dst: int, async_op=False):
    ...
    if not async_op:
        CudaTimer().start(field_name='move', predefined=True, comm_ranks=[src, dst], comm_size=tensor_size)
    rank = torch.distributed.get_rank()
    work = None
    if rank == src:
        tensor = tensor.contiguous() if not tensor.is_contiguous() else tensor
        ...
        if async_op:
            ...
            torch.distributed.recv(tensor, src)
    if not async_op:
        CudaTimer().stop(field_name='move', predefined=True)
    return tensor
...
def rdscatter(itensor: torch.Tensor, shape: Tuple[int], dtype: torch.dtype,
    ...
    """
    tensor_size = 0
    if itensor is not None:
        tensor_size = _get_tensor_size(itensor)
    elif shape and dtype:
        tensor_size = 1
        for s in shape:
            tensor_size *= s
    ...
    if async_op:
        CudaTimer().start(field_name='rdscatter', predefined=True, comm_ranks=[src] + list(dsts), comm_size=tensor_size)
    rank = torch.distributed.get_rank()
    if rank == src:
        with torch.no_grad():
    ...
    if async_op:
        CudaTimer().stop(field_name='rdscatter', predefined=True)
    return otensor
...
def rvscatter(itensor: torch.Tensor, shape: Tuple[int], dtype: torch.dtype,
    ...
    if not async_op:
        CudaTimer().start(field_name='rvscatter', predefined=True, comm_ranks=[src] + list(dsts), comm_size=tensor_size)
    ...
    if work:
        AsyncCommHandler().submit(tensor, [work])
    if not async_op:
        CudaTimer().stop(field_name='rvscatter', predefined=True)
    return tensor
...
def rdgather(itensor: torch.Tensor, shape: Tuple[int], dtype: torch.dtype,
    ...
    if not async_op:
        CudaTimer().start(field_name='rdgather', predefined=True, comm_ranks=list(srcs) + [dst], comm_size=tensor_size)
    rank = torch.distributed.get_rank()
    if rank == dst:
        recv_tensors, works = [], []
        for src in srcs:
        ...
        if async_op:
            ...
            torch.distributed.send(otensor, dst)
    if not async_op:
        CudaTimer().stop(field_name='rdgather', predefined=True)
    return otensor
...
def rvgather(itensor: torch.Tensor, shape: Tuple[int], dtype: torch.dtype,
    ...
    if not async_op:
        CudaTimer().start(field_name='rvgather', predefined=True, comm_ranks=list(srcs) + [dst], comm_size=tensor_size)
    rank = torch.distributed.get_rank()
    ...
    if work and rank == dst:
        AsyncCommHandler().submit(tensor, [work])
    if not async_op:
        CudaTimer().stop(field_name='rvgather', predefined=True)
    return tensor
...
def broadcast(itensor: torch.Tensor, shape: Tuple[int], dtype: torch.dtype, src: int, ranks: List[int], async_op=False) -> torch.Tensor:
    ...
    if not async_op:
        CudaTimer().start(field_name='broadcast', predefined=True, comm_ranks=ranks, comm_size=tensor_size)
    rank = torch.distributed.get_rank()
    group = DeviceGroup().get_group(ranks)
    if rank == src:
    ...
    if work and rank != src:
        AsyncCommHandler().submit(tensor, [work])
    if not async_op:
        CudaTimer().stop(field_name='broadcast', predefined=True)
    return tensor
