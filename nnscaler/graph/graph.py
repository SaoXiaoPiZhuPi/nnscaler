...
class IRGraph(IRSegment):
    ...
    def percur(self, node: IRCell):
        ...
        Returns:
            List[IRcell]: the Precursor nodes of a given node
        """
        precursors = []
        if isinstance(node,IRAdapter):
            input_tensors=node._inputs
            for input in input_tensors:
                prior_nodes=self.producers(input.parent)
                assert isinstance(prior_nodes, tuple), "inputs of IRAdapter should be a list"
                precursors.append(prior_nodes)
        else:
            for input in node.iobjs():
                prior_nodes=self.producers(input.parent)
                assert isinstance(prior_nodes, tuple), "inputs of node should be a tuple"
                precursors.append(prior_nodes)
        return precursors
        

    def subseq(self, node: IRCell):
        """find Subsequent nodes of a given node
        Args:
            node: given node
        
        Returns:
            List[IRcell]: the Subsequent nodes of a given node
        """
        Subsequent_nodes=[]
        if isinstance(node,IRAdapter):
            output_tensors=node._outputs
            for out in output_tensors:
                next_nodes=self.consumers(out.parent)
                assert isinstance(next_nodes, tuple), "outputs of IRAdapter should be a list"
