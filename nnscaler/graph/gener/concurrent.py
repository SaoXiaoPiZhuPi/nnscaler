...
class ConcurrentGener:
    ...
    def gen_intra_rvd(fptensors: List[IRSubTensor], fctensors: List[IRSubTensor], 
        ...
        f=open('./log/data_map.txt','a')
        print("\nProducers and their devices:",file=f)
        for ptensor in fptensors:
            print(f"  SubTensor: {ptensor}, Device: {ptensor.device}",file=f)
        ...
        # 打印消费者信息
        print("Consumers and their devices:",file=f)
        for ctensor in ctensors:
            print(f"  SubTensor: {ctensor}, Device: {ctensor.device}",file=f)
        f.flush()
