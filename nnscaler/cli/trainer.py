...
class Trainer:
    ...
    def run(self):
        ...
        logging.info(f'编译时间为：{compile_end-compile_start}')
        ...
        if is_running_distributed():
            comm_log_dir = Path('/data/haiqwa/zevin_nfs/andy/Auto-Parallelization/nnscaler_group1/nnscaler-h3c/examples/logs') / 'comm_logs'
            comm_log_dir.mkdir(parents=True, exist_ok=True)
            comm_log_file = comm_log_dir / f'comm_logs_rank{torch.distributed.get_rank()}.json'
            CudaTimer(enable=True, predefined=True)
            CudaTimer().enable_comm_logging(str(comm_log_file))
