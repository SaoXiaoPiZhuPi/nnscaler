...
public:
  ...
  void init_dp_info() {
    for (auto iter = id2node.begin(); iter != id2node.end(); ++iter) {
      ...
      printf("cut num: %d dp_num: %d p_num: %d producer_num: %d\n",
             node->cut_len, node->dp_num, node->p_num, node->producer_num);
  ...
  void update(DPNode *dp_node, int start_level) {
    ...
    while (!pq.empty()) {
      ...
      if (dp_node->state.empty() && cur_state.total_mem <= mem_bound) {
        ...
        // if (cur_mem <= mem_bound && cur_cost < pre_cost) {
        //   if (cur_mem > pre_mem) {
        //     dp_node->state.push_back(cur_state);
        //   } else {
        //     dp_node->state[dp_node->state.size() - 1] = cur_state;
        //   }
        // }
    ...
    if (!flag && cnt > 1000) {
      ...
      for (std::size_t i = 0; i < dp_node->in_edges.size(); ++i) {
        ...
        double transition_cost = dp_node->in_edges[i].second;
        UnitDPState new_state = pred_state.generate_new_state(
            node, cur_p, transition_cost, pred, 0);
        pq.push(std::make_tuple(new_state, i));
      }
      ...
      while (!pq.empty()) {
        ...
        printf("cur_state: %lf %lf\n", cur_state.total_mem / 2.0,
               cur_state.time_cost);
      }
    }
  }
  ...
  void do_dp(int start_level, int end_level) {
    ...
    for (int i = start_level; i <= end_level; ++i) {
      ...
      std::vector<std::pair<int, int>> split_info =
          split_work(iter->second->dp_num, MAX_CONCURRENCY);
    ...
    int Max = mp1.rbegin()->first;
    puts("mp1:----------------------------");
    for (int i = 0; i <= Max; i++) {
      printf("%d %d\n", i, mp1[i]);
    }
    puts("mp2:----------------------------");
    Max = mp2.rbegin()->first;
    for (int i = 0; i <= Max; i++) {
      printf("%d %d\n", i, mp2[i]);
    }
    puts("----------------------------");
  ...
  SearchPlan process_state(DPNode *dp_node, int idx) {
    ...
    return SearchPlan{best_state.time_cost, best_state.total_mem, path};
  }
...
PYBIND11_MODULE(dp_solver_test, m) {
